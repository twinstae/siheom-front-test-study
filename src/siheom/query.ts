...
export const getElement = <T extends boolean>(
  ...
  if (locator.role === "text") {
    if (isVisible) {
      return screen.getByText(locator.name);
    }
    return screen.queryByText(locator.name) as HTMLElement;
  }
...
export const getElements = <T extends boolean>(
  ...
  if (locator.role === "text") {
    if (isVisible) {
      return screen.getAllByText(locator.name);
    }
    return screen.queryAllByText(locator.name);
  }
